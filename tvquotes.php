...
$dbname="user_registration";
    ...
    $sql = "SELECT * from Quotes where Category='TV Series'";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td style=\"text-align: center; vertical-align: middle;\" ><button class=\"btn\" type=\"submit\" value=\"button\" onclick=\"saveQuote('". $row['Origin'] ."','". $row['Quote'] ."','TV Series','". $row['Rating'] ."')\">Add</button></td>";;
            echo "</tr>";
        }
    }
    $conn->close();
